...
struct fwm_event_handlers fwm_event_handlers;

void fwm_initialize_event_handlers(void) {
	fwm_event_handlers.map_request = fwm_event_map_request;
}

void fwm_handle_event(xcb_generic_event_t *event) {
	switch (event->response_type) {
		case XCB_MAP_REQUEST:
			fwm_event_handlers.map_request((xcb_map_request_event_t*) event);
		default:
			break;
	}
}
